...
static void uart1_irq_handler(const struct device *dev, void *user_data)
{
...

{
    ...
    tx_data = buf;
    tx_data_length = len;
    uart_irq_callback_set(dev, uart1_irq_handler);
...
int main(void)
{
	...
	app_uart1_init();
	while(1){
		printk("Sending message from UART1\n");
		char* msg = "Hello from UART1\n";
		//int err = uart_tx(dev_uart1, &msg, 17, 1000);
		//int err = uart_fifo_fill(dev_uart1, (uint8_t*)msg, 17);
		// if(err != 0){
		// 	printk("Error sending message from UART1 = %i\n",err);
		// }
